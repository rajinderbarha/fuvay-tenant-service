...
This slice is domain-integrity work on `create_job`'s service-layer logic — it does not change
any router dependency, permission, or guard. No route classification changed.
...
- Canonical global: **169 protected of 210** — recomputed directly from the master CSV, identical
  to the Slice 2F-14C baseline (no rows added, removed, or reclassified).
...
(unmodified this slice — no assertion needed updating, since the numbers are unchanged) continues
to pass: `total == 210`, `protected == 169`, field_ops `== 40/40`.
