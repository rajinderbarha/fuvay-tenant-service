...
# Carried forward — Slice 2F-26H

All 28 prior observations retained. **None remediated; zero application files
modified; all static-reading only — none executed against a live tenant pair.**

Kept prominent (as WS20 requires):

- `POST /v1/security/sessions/{session_id}/revoke` — revokes by session id, principal never referenced
- `delete_rule` / `delete_kb` / `delete_asset` — tenant discarded or absent before the service call
- `POST /v1/commerce/tenants/{tenant_id}/badges/recalculate` — read permission gating a write
- `POST /v1/security/audit-log` — arbitrary authenticated audit-log insertion
- client-asserted tenant identifiers without proven ownership

## New this slice (from fifth-holdout reading, static only)

| Route | Static evidence | Confidence | Executed |
|---|---|---|---|
| `POST /v1/security/api-keys/{key_id}/revoke` | `require_permission(TENANT_UPDATE)`, Query tenant_id; no own-tenant assertion | MEDIUM | no |
| `POST /v1/security/api-keys/{key_id}/rotate` | same shape; mints a new key | MEDIUM | no |
| `DELETE /v1/rag/documents/{doc_id}` | `delete_document(doc_id)` — tenant never passed; same family as delete_rule/delete_kb | MEDIUM | no |
| `POST /v1/rag/documents/{doc_id}/reindex` | object by id, no tenant | LOW | no |
| `POST /v1/catalog` / `PUT /v1/catalog/{item_id}` | tenant from body (POST) or absent (PUT); no assertion | MEDIUM | no |
| `POST /v1/notifications/send` | `tenant_id` + `recipient_id` from body under `get_current_user` | MEDIUM | no |
| `POST /v1/media/upload/initiate` | `tenant_id` from body | MEDIUM | no |
| `POST /v1/inventory/items/{item}/locations/{loc}/receive` | `tenant_id` from body; adjusts stock | MEDIUM | no |
| `PUT /v1/settings/tenants/{tenant_id}/{key}` | client tenant, arbitrary key/value | MEDIUM | no |

The `delete_document` / `delete_rule` / `delete_kb` / `delete_asset` family
remains the single highest-signal group: objects addressed by id with the
tenant either discarded or absent. Each needs a runtime two-tenant test before
