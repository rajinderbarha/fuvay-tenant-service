...
RUN npm run build --workspace=frontend/tenant-portal
...
COPY --from=builder /app/frontend/tenant-portal ./frontend/tenant-portal

WORKDIR /app/frontend/tenant-portal

EXPOSE 3001

CMD ["/app/node_modules/.bin/next", "start", "-p", "3001", "-H", "0.0.0.0"]
