...
import {
  getCustomerBookingDetail, getCustomerBookingTracking, getCancelRescheduleEligibility,
  getTrackingLocation, CancelRescheduleEligibility, TrackingLocation,
...
export default function BookingDetailPage() {
  ...
  const load = () => {
    ...
    getCancelRescheduleEligibility(bookingId).then(setElig).catch(() => setElig(null));
    getTrackingLocation(bookingId).then(setLoc).catch(() => setLoc(null));
  };
    ...
    <div className="co-container">
      ...
      {detail && !detail.error && (
        <div className="co-card" style={{ display: "flex", flexDirection: "column", gap: 10, marginBottom: 16 }}>
          ...
          {/* TRACK-TECHNICIAN: only shown when the backend confirms a real,
              fresh-enough location actually exists for this job right now —
              never inferred from job status alone. */}
          {loc?.available && (
            <Link href={`/customer/bookings/${bookingId}/track`} className="co-btn-primary"
              style={{ background: "var(--danger)", boxShadow: "none", textAlign: "center" }}>
              Track technician
            </Link>
          )}
          {/* CANCEL-RESCHEDULE-FOUNDATION: entry point only shown when the
              backend's own eligibility check permits at least one action —
              never inferred from a locally mirrored status set. */}
          {(elig?.can_cancel || elig?.can_reschedule) && (
            <Link href={`/customer/bookings/${bookingId}/manage`} className="co-btn-secondary" style={{ textAlign: "center" }}>
              Manage booking
            </Link>
          )}
