...
export default function ChecklistDetailPage(){
  ...
  const retired=template.status==="archived";
  return <AdminLayout activeNav="checklist-templates">
    ...
    {tab==="content"&&<Card><ChecklistTemplateEditor key={String(id)} templateId={String(id)} retired={retired} onChanged={()=>{detail.refetch();latest.refetch();audit.refetch();}}/></Card>}
    ...
    {tab==="activity"&&<Card><h3 style={{marginTop:0}}>Audit activity</h3>{audit.error&&<p role="alert">{audit.error}</p>}{(audit.data?.items??[]).length===0?<p style={{fontSize:13,color:"var(--text-tertiary)"}}>No audited changes yet.</p>:(audit.data?.items??[]).map(event=><div key={event.id} style={{display:"grid",gridTemplateColumns:"140px 1fr auto",gap:12,padding:"11px 0",borderBottom:"1px solid var(--border)",alignItems:"center"}}><Badge variant="muted">{event.action}</Badge><div><div style={{fontSize:13}}>{event.change_summary||"Checklist changed"}</div><div style={{fontSize:11,color:"var(--text-tertiary)"}}>{event.actor_role||"system"}{event.request_id?` · ${event.request_id}`:""}</div></div><span style={{fontSize:11,color:"var(--text-tertiary)"}}>{event.created_at?new Date(event.created_at).toLocaleString():"—"}</span></div>)}</Card>}
