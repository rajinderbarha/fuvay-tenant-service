...
function OverviewTab({ range, setRange, home, trends, requestDemand, actions, activity, resolve, snooze, busy }: { range: Range; setRange: (r: Range) => void; home: ApiState; trends: ApiState; requestDemand: ApiState; actions: ApiState; activity: ApiState; resolve: (id: string) => void; snooze: (id: string) => void; busy: boolean }) {
  return <div className={styles.workspace}><div className={styles.primaryColumn}>
    ...
    <Card><SectionTitle title="Home Services Summary" description="Live production gates from tenant setup, bookability, finance, and trust controls."/>{home.error ? <SectionError title="Readiness unavailable" error={home.error} requestId={home.requestId} onRetry={home.refetch}/> : home.loading ? <Skeleton height={270}/> : <div className={styles.readinessList}>{[
      ["Service Catalog", home.data?.service_catalog_health.status, "/admin/catalog-workspace"], ["Finance Rules", home.data?.pricing_rule_health.status, "/admin/home-services/finance?tab=monetization"], ["Provider Coverage Readiness", home.data?.provider_coverage_health.status, "/admin/home-services/providers"], ["Bookability & Trust Gates", home.data?.provider_bookability_health.status, "/admin/home-services/providers"], ["Completion Deductions", home.data?.completed_job_deduction_health, "/admin/home-services/finance?tab=provider-charges"],
    ].map(([label, status, href]) => <Link href={String(href)} key={String(label)} className={styles.readinessItem}><span className={status === "healthy" ? styles.stateGood : status === "warning" ? styles.stateWarn : styles.stateBad}>{status === "healthy" ? <CheckCircle2 size={15}/> : <AlertTriangle size={15}/>}</span><span><strong>{label}</strong><small>{label === "Bookability & Trust Gates" ? `${String(status || "not configured").replaceAll("_", " ")} · ${home.data?.provider_bookability_health.bookable_providers ?? 0}/${home.data?.provider_bookability_health.eligible_providers ?? 0} approved providers bookable${home.data?.awaiting_verification_providers ? ` · ${home.data.awaiting_verification_providers} awaiting verification` : ""}` : String(status || "not configured").replaceAll("_", " ")}</small></span><ArrowRight size={13}/></Link>)}</div>}</Card>
