  ...
  return <Card>
    ...
    {!areas.length ? <div className={styles.chartEmpty}><FileBarChart size={23}/><strong>No booking demand in this period.</strong><span>Confirmed bookings and failed customer-app, Instagram and WhatsApp coverage checks will appear here.</span></div> : <>
      <div className={styles.demandSignals}>
        ...
        <div><span><MapPin size={13}/>ZIP captured</span><strong>{Number(demand.data?.location_capture_pct ?? 0)}%</strong><small>no customer PII stored</small></div>
      </div>
...
export default function PlatformCommandCenterPage() {
  ...
  return <AdminLayout activeNav="dashboard"><main className={styles.page}>
    ...
    {summary.loading || health.loading ? <div className={styles.kpiGrid}>{Array.from({ length: 5 }).map((_, i) => <Skeleton key={i} height={126}/>)}</div> : <div className={styles.kpiGrid}>
      <Kpi label="Platform Health" value={`${h?.score ?? 0}/100`} note={(h?.status || "unknown").replaceAll("_", " ")} icon={<HeartPulse size={18}/>} tone={(h?.score ?? 0) >= 90 ? "success" : (h?.score ?? 0) >= 70 ? "warning" : "danger"} href="/admin/engines"/>
      ...
      <Kpi label="Pending Admin Actions" value={s?.pending_admin_actions.count ?? 0} note={`${s?.pending_admin_actions.approvals ?? 0} approvals · ${s?.pending_admin_actions.finance_exceptions ?? 0} finance · ${s?.pending_admin_actions.security_alerts ?? 0} security`} icon={<ClipboardCheck size={18}/>} tone={(s?.pending_admin_actions.count ?? 0) ? "warning" : "success"} href="/admin/operations"/>
...
function OverviewTab({ range, setRange, home, trends, requestDemand, actions, activity, resolve, snooze, busy }: { range: Range; setRange: (r: Range) => void; home: ApiState; trends: ApiState; requestDemand: ApiState; actions: ApiState; activity: ApiState; resolve: (id: string) => void; snooze: (id: string) => void; busy: boolean }) {
  const deductionHealth = home.data?.completed_job_deduction_health;
  const deductionStatus = typeof deductionHealth === "string" ? deductionHealth : deductionHealth?.status;
  const readinessItems = [
    { label: "Service Catalog", status: home.data?.service_catalog_health.status, detail: `${home.data?.service_catalog_health.active_services ?? 0} active services`, href: "/admin/catalog-workspace" },
    { label: "Finance Rules", status: home.data?.pricing_rule_health.status, detail: `${home.data?.pricing_rule_health.active_rules ?? 0} published rules`, href: "/admin/home-services/finance?tab=monetization" },
    { label: "Provider Coverage Readiness", status: home.data?.provider_coverage_health.status, detail: `${home.data?.provider_coverage_health.tenants_without_areas ?? 0} providers without coverage`, href: "/admin/home-services/providers" },
    { label: "Bookability & Trust Gates", status: home.data?.provider_bookability_health.status, detail: `${home.data?.provider_bookability_health.bookable_providers ?? 0}/${home.data?.provider_bookability_health.eligible_providers ?? 0} approved providers bookable${home.data?.awaiting_verification_providers ? ` · ${home.data.awaiting_verification_providers} awaiting verification` : ""}`, href: "/admin/bookability/providers" },
    { label: "Completion Deductions", status: deductionStatus, detail: `${deductionHealth && typeof deductionHealth === "object" ? deductionHealth.missing_deductions : 0} missing in the last 30 days`, href: "/admin/home-services/finance?tab=provider-charges" },
  ];
  return <div className={styles.workspace}><div className={styles.primaryColumn}>
    ...
    <Card><SectionTitle title="Home Services Summary" description="Live production gates from tenant setup, bookability, finance, and trust controls."/>{home.error ? <SectionError title="Readiness unavailable" error={home.error} requestId={home.requestId} onRetry={home.refetch}/> : home.loading ? <Skeleton height={270}/> : <div className={styles.readinessList}>{readinessItems.map(item => <Link href={item.href} key={item.label} className={styles.readinessItem}><span className={item.status === "healthy" ? styles.stateGood : item.status === "warning" ? styles.stateWarn : styles.stateBad}>{item.status === "healthy" ? <CheckCircle2 size={15}/> : <AlertTriangle size={15}/>}</span><span><strong>{item.label}</strong><small>{String(item.status || "not configured").replaceAll("_", " ")} · {item.detail}</small></span><ArrowRight size={13}/></Link>)}</div>}</Card>
...
function OperationsTab({ data, live, actions, search, setSearch, visible, resolve, snooze, busy }: { data: ApiState; live: ApiState; actions: ApiState; search: string; setSearch: (s: string) => void; visible: any[]; resolve: (id: string) => void; snooze: (id: string) => void; busy: boolean }) {
  return <div className={styles.workspaceSingle}>{data.error ? <SectionError title="Operations summary unavailable" error={data.error} requestId={data.requestId} onRetry={data.refetch}/> : <div className={styles.fourGrid}><Kpi label="Live Jobs" value={data.data?.live_jobs ?? 0} note="In native execution" icon={<Wrench size={18}/>}/><Kpi label="Today's Bookings" value={data.data?.today_bookings ?? 0} note="Confirmed today across channels" icon={<CalendarClock size={18}/>}/><Kpi label="Awaiting Technician" value={data.data?.pending_provider_acceptance ?? 0} note="Provider must assign staff" icon={<UsersRound size={18}/>} tone={(data.data?.pending_provider_acceptance ?? 0) ? "warning" : "success"}/><Kpi label="Job SLA Breaches" value={data.data?.sla_breaches ?? 0} note={`${data.data?.complaint_sla_breaches ?? 0} complaint SLA breaches`} icon={<BellRing size={18}/>} tone={(data.data?.sla_breaches ?? 0) || (data.data?.complaint_sla_breaches ?? 0) ? "danger" : "success"}/></div>}
    <Card><SectionTitle title="Live operations board" description="Current native Home Services jobs; use the full workspace for bulk action." action={<div className={styles.inlineActions}><label className={styles.search}><Search size={14}/><input value={search} onChange={event => setSearch(event.target.value)} placeholder="Search job, provider, status"/></label><Link href="/admin/home-services/bookings-jobs"><Btn size="sm">Full workspace <ArrowRight size={13}/></Btn></Link></div>}/>{live.error ? <SectionError title="Live operations unavailable" error={live.error} requestId={live.requestId} onRetry={live.refetch}/> : live.loading ? <Skeleton height={280}/> : <DataTable headers={["Job", "Provider", "Status", "SLA", "Assignment", "Updated"]} empty={!visible.length}>{visible.map(item => <tr key={item.id}><td><Link href={`/admin/home-services/bookings-jobs?job_id=${item.id}`} className={styles.entityLink}>{item.item}</Link></td><td>{item.tenant || "Unassigned"}</td><td><Badge variant="info">{item.status.replaceAll("_", " ")}</Badge></td><td><Badge variant={item.sla_breach ? "danger" : "success"}>{item.sla_breach ? "Breached" : "On track"}</Badge></td><td>{item.assigned_to ? "Assigned" : "Unassigned"}</td><td>{item.updated_at ? new Date(item.updated_at).toLocaleString("en-IN") : "—"}</td></tr>)}</DataTable>}</Card>
...
function PlatformTab({ engines, compliance, trust, categories, permissions }: { engines: ApiState; compliance: ApiState; trust: ApiState; categories: ApiState; permissions: ReturnType<typeof usePermissions> }) {
  return <div className={styles.workspace}><div className={styles.primaryColumn}>
    ...
    <Card><SectionTitle title="Quick Links"/><div className={styles.quickLinks}>{([[
      "Tenant Approvals", "/admin/home-services/providers?status=pending_review", "tenant:read"], ["Live Operations", "/admin/home-services/bookings-jobs", "admin:jobs:read"], ["Finance Summary", "/admin/home-services/finance", "finance:hub:read"], ["Analytics Reports", "/admin/analytics?tab=reports", "analytics:dashboard:read"], ["Security Center", "/admin/security", "security:read"], ["Engine Health", "/admin/engines", SUPER_ADMIN_ONLY],
