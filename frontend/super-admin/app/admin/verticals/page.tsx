    ...
    <div className="vertical-table-shell">
      <div className="vertical-table-scroll"><TableSurface className="vertical-table"><thead><tr>
        ...
        {query.loading ? Array.from({ length: 5 }).map((_, i) => <tr key={i}><td colSpan={9}><div className="skeleton" style={{ height: 38, margin: 7 }} /></td></tr>) : items.map(v => <tr key={v.id}>
          {visible.has("label") && <td><div className="vertical-name"><span className="vertical-icon" style={{ color: v.color ?? "var(--brand)", background: `${v.color ?? "#0f6b60"}16` }}><Globe size={16} /></span><div><Link className="vertical-link" href={`/admin/verticals/${v.key}`}>{v.label}</Link><div className="vertical-code">{v.key}</div></div></div></td>}
