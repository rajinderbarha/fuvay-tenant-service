...
export default function ProviderDetailPage() {
  ...
  async function load() {
    setLoading(true);
    try {
      const r = await adminAnalyticsApi.getProviderDetail(id, { date_from: df, date_to: dt });
      setData(r?.data ?? null);
    } catch { setData(null); }
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(180px,1fr))", gap: 16 }}>
        ...
        <AnalyticsKpiCard label="Commission (AED)" value={summary.total_commission} loading={loading} />
        <AnalyticsKpiCard label="Avg Rating"       value={summary.avg_rating}       loading={loading} />
      </div>

      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(320px,1fr))", gap: 24 }}>
        <div style={card}>
          <h2 style={h2}>Staff Performance</h2>
          <AnalyticsBreakdownTable
            rows={topItems}
            loading={loading}
            columns={[
        ...
        <div style={card}>
          <h2 style={h2}>Monthly Breakdown</h2>
          <AnalyticsBreakdownTable
            rows={breakdown}
            loading={loading}
            columns={[
