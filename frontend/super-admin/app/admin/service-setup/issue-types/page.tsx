...
export default function IssueTypesPage() {
  ...
  const [form, setForm] = useState({ name: "", code: "", description: "", severity: "medium", vertical_type: "", requires_photo: false, requires_description: false });
  ...
  async function handleCreate(event: React.FormEvent) {
    ...
    setFormError("");
    try {
      await serviceOptionApi.createIssueType({ ...form, name: form.name.trim(), code: form.code.trim(), description: form.description.trim(), vertical_type: form.vertical_type.trim(), severity_default: form.severity } as Parameters<typeof serviceOptionApi.createIssueType>[0]);
      setShowCreate(false);
      setForm({ name: "", code: "", description: "", severity: "medium", vertical_type: "", requires_photo: false, requires_description: false });
      setActionMessage("Issue type created.");
      await load();
    } catch (error) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "var(--layout-page-gap)" }}>
      <PageHeader eyebrow="Catalog" context="Service setup" title="Issue Types" description={`${total.toLocaleString()} problem categories available to booking flows.`}
        actions={<Btn onClick={() => { setFormError(""); setShowCreate(true); }}><Plus size={14} /> New issue type</Btn>} />
      ...
      <div>
        <DataTable<IssueRow> loading={loading} rows={items as IssueRow[]} emptyText="No issue types match the current filters." columns={[
          { key: "name", label: "Issue type", render: (value, row) => <div><div style={{ fontWeight: 650 }}>{String(value)}</div><div style={{ fontSize: 11, color: "var(--text-tertiary)", marginTop: 2 }}>{row.description || "No description"}</div></div> },
      ...
      <Modal open={showCreate} onClose={() => !saving && setShowCreate(false)} title="New issue type" size="lg">
        <form onSubmit={handleCreate} style={{ display: "flex", flexDirection: "column", gap: "var(--space-4)" }}>
          ...
          <div style={{ display: "flex", justifyContent: "flex-end", gap: "var(--space-2)" }}><Btn type="button" variant="secondary" disabled={saving} onClick={() => setShowCreate(false)}>Cancel</Btn><Btn type="submit" loading={saving}>Create issue type</Btn></div>
        </form>
      </Modal>
    </div>
  );
