...
export function RuntimeTemplatesPanel() {
  ...
  return <div className="nc-stack">
    <div className="nc-settings-hero"><div><span className="nc-eyebrow">PLATFORM EVENT MESSAGES</span><h2>Administrator and system alerts</h2><p>Manage the in-app messages created by platform events and delivered through the notification outbox.</p></div><div style={{ display: "flex", gap: 9 }}><div className="nc-health-chip"><strong>{active}</strong><span>active of {templates.data?.total ?? 0}</span></div><Btn size="sm" onClick={create}><Plus size={14}/> New template</Btn></div></div>
    ...
    <div className="nc-callout"><CheckCircle2 size={17}/><div><strong>Connected to the event outbox</strong><p>These templates cover platform-generated in-app alerts. Customer chat, field-service, and reminder messages are managed in the library above.</p></div></div>
