"use client";
/**
 * PortalKit — the component set that super-admin and tenant-portal each
 * independently reimplemented in their own `components/shared/ui.tsx`.
 *
 * Both files were ~90% identical and had genuinely drifted (button corner
 * radius, IconBtn hover colors, Input focus styling, DataTable's dropdown-
 * clipping fix present in one but not the other) -- exactly the kind of
 * silent inconsistency the earlier KPI-card unification (SummaryCard/
 * StatCard, see git history) was already fixing piecemeal. This is the
 * single real implementation both apps now import; their local ui.tsx
 * files are thin re-exports so no call site in either app had to change.
 *
 * Several of these names collide with OLDER, already-real primitives this
 * package exports (Card, Modal, Skeleton, Avatar, StatCard, DataTable,
 * Spinner) that dozens of production pages already depend on with a
 * DIFFERENT prop shape (e.g. package Button's `leftIcon`/`rightIcon` +
 * variant set vs this file's `icon`/`iconRight` + variant set; package
 * DataTable's required `rowKey` + sortable columns vs this file's simpler
 * `render`-only columns). Forcing those into one API would mean rewriting
 * every call site on both sides -- so the colliding ones are exported here
 * under a distinct name (SurfaceCard, Dialog, TextSkeleton, InitialsAvatar,
 * MetricCard, SimpleTable, LoadingSpinner) and each app's local ui.tsx
...
export function Btn({
  ...
  const S: Record<PkBtnSize, React.CSSProperties> = {
  ...
  return (
    <button {...buttonProps} className={["pk-btn", buttonProps.className].filter(Boolean).join(" ")} type={type} onClick={onClick} disabled={disabled || loading}
      ...
      {loading ? <LoadingSpinner size={13}/> : icon}
      {children}
      {!loading && iconRight}
    </button>
  );
...
// ── PkBadge (small pill badge; distinct from the package's own StatusBadge) ─
export type PkBadgeVariant = "default" | "success" | "warning" | "danger" | "info" | "muted" | "golden" | "terra";
export function PkBadge({ children, variant, tone, size = "md", dot }: {
  children: React.ReactNode;
  variant?: PkBadgeVariant;
  /** Alias for `variant` -- some pages were written against `tone`
   * (matching the mobile design system); accepting both keeps one Badge
   * rather than two divergent ones. */
  tone?: PkBadgeVariant | string;
  size?: "sm" | "md" | "lg"; dot?: boolean;
}) {
  const resolved: PkBadgeVariant = (variant ?? (tone as PkBadgeVariant) ?? "default");
  const V: Record<PkBadgeVariant, React.CSSProperties> = {
...
// ── PkInput (distinct from the package's own `Field`) ────────────────────
// `rows` gives an inline textarea variant -- three real tenant-portal
// pages pass it, so it must stay even though super-admin's original
// version never had it.
export function PkInput({ label, placeholder, value, onChange, error, hint, icon, type = "text", disabled, required, rows }: {
...
// ── PkSelect ───────────────────────────────────────────────────────────────
export function PkSelect({ label, value, onChange, options, placeholder, disabled }: {
...
// ── LoadingSpinner (distinct from the package's own `Spinner`) ──────────
export function LoadingSpinner({ size = 20, color = "var(--accent)" }: { size?: number; color?: string }) {
  return (
...
// ── TextSkeleton (distinct from the package's own `Skeleton`) ───────────
export function TextSkeleton({ width, height = 20, radius = 6, style = {} }: {
   ...
   * ("In HTML, <div> cannot be a descendant of <p>"). This exact bug took
   * out the tenant Dashboard once already -- the package's own `Skeleton`
   * renders a <div> and does not have this fix, so pages that need a
   * text-line placeholder should use this one instead.
   */
  return (
    <span
      className="skeleton"
...
// ── InitialsAvatar (distinct from the package's own `Avatar`) ───────────
const PK_AVATAR_PAL = [
...
function pkHashIdx(s: string) { return [...s].reduce((h, c) => (h * 31 + c.charCodeAt(0)) % PK_AVATAR_PAL.length, 0); }
export function InitialsAvatar({ name, size = 36 }: { name: string; size?: number }) {
  const p = PK_AVATAR_PAL[pkHashIdx(name)];
...
// ── Dialog (distinct from the package's own `Modal`) ─────────────────────
// Full focus-trap + focus-restoration a11y implementation, from the
// FINAL-L5-05AC super-admin accessibility certification pass.
let pkDialogTitleSeq = 0;
export function Dialog({ open, onClose, title, children, size = "md" }: {
  ...
  const titleId = React.useRef(`pk-dialog-title-${++pkDialogTitleSeq}`).current;
...
// ── Toast ─────────────────────────────────────────────────────────────────
export interface PkToastItem { id: string; title: string; description?: string; variant: "success" | "warning" | "danger" | "info"; }
const PK_TOAST_CONFIG = {
...
export function PkToaster({ toasts, onRemove }: { toasts: PkToastItem[]; onRemove: (id: string) => void }) {
  return (
    <div
      role="status"
      aria-live="polite"
    ...
    >
      {toasts.map(t => {
        const c = PK_TOAST_CONFIG[t.variant];
        return (
          <div key={t.id} style={{
...
// ── SummaryCard ───────────────────────────────────────────────────────────
// Canonical compact KPI tile. Unified across super-admin and tenant-portal
// 2026-08-05 (see git history "unify KPI cards") after ~20 pages across
// both portals had each declared their own local, drifted SummaryCard.
...
export function SummaryCard({
   ...
   *  (several call sites pass a raw colour rather than a flag). */
...
// ── SimpleTable (distinct from the package's own `DataTable`) ────────────
// Keeps the overflow:visible fix from super-admin's version -- overflow:
// hidden on the wrapper clipped any in-cell dropdown menu that extended
// past the table's own bounds, since no z-index on the dropdown can
// override an ancestor's overflow:hidden. Rounded corners come from the
// <table> itself instead.
function PkDtRow<T extends Record<string, unknown>>({ row, columns, index, rowCount, onRowClick }: {
  row: T;
    ...
    <div className="ds-simple-table" style={{
      ...
      <table style={{ width: "100%", borderCollapse: "collapse" }}>
        ...
        <tbody>
          {loading
            ? [...Array(5)].map((_, i) => (
                <tr key={i}><td colSpan={columns.length} style={{ padding: "12px 16px" }}><TextSkeleton height={18}/></td></tr>
              ))
            : rows.length === 0
            ? (
...
// ── HealthMeter ────────────────────────────────────────────────────────────
const PK_HEALTH_COLORS: { [k: string]: { bg: string; text: string; border: string; icon: string; label: string } } = {
...
function pkGetHealthBand(score: number): string {
...
export function HealthMeter({ score }: { score: number }) {
  const c = PK_HEALTH_COLORS[pkGetHealthBand(score)];
  return (
...
// ── PkTextarea (distinct from the package's own `Field`) ─────────────────
export function PkTextarea({ label, placeholder, value, onChange, hint, error, rows = 4, disabled, required }: {
