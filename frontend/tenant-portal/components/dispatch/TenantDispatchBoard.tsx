...
import { AlertTriangle, CalendarClock, ChevronLeft, ChevronRight, UserRoundCheck, UserRoundX } from "lucide-react";
import type {
...
function jobsForDay(technician: HsDispatchTechnician, date: string) {
  ...
  return jobs.filter(job => (job.scheduled_date || job.requested_date) === date);
}
...
function AssignedJobCard({ job, selected, onSelect, breach = false }: {
  ...
  return <button type="button" className="tenant-dispatch-assigned-card" data-breach={breach} aria-current={selected} onClick={onSelect}>
    <ServiceJobSummary job={job} due={due} />
    <span className="tenant-dispatch-assigned-meta">
      <span><UserRoundCheck size={13} />{job.assigned_staff_name || "Assigned technician"}</span>
      <b>{statusLabel(job.status)}</b>
    </span>
  </button>;
}

function ServiceJobSummary({ job, due }: {
  job: HsDispatchJobSummary;
  due: ReturnType<typeof urgency>;
}) {
  return <>
    ...
    <span>{slotLabel(job)}{job.locality ? ` · ${job.locality}` : ""}</span>
  </>;
}
  ...
  return <article className="tenant-dispatch-technician">
    ...
    <div className="tenant-dispatch-slots">
      {jobs.map(job => <button type="button" key={job.job_id} aria-current={selectedJobId === job.job_id} onClick={() => onSelect(job.job_id)}>
        <strong>{slotLabel(job)}</strong><span>{job.job_number} · {serviceLabel(job)}</span><small data-overdue={job.is_overdue}>{job.is_overdue ? "SLA breached" : statusLabel(job.status)}</small>
      </button>)}
  ...
  return <div className="tenant-dispatch-reference">
    ...
    <section className="tenant-dispatch-section tenant-dispatch-assigned">
      ...
      <div className="tenant-dispatch-assigned-grid">
        ...
        {!assignedJobs.length && <div className="tenant-dispatch-empty">No assigned jobs for this date.</div>}
      </div>
    </section>
    ...
    <section className="tenant-dispatch-section tenant-dispatch-breached">
      ...
      <div className="tenant-dispatch-assigned-grid">
        ...
        {!breachedJobs.length && <div className="tenant-dispatch-empty">No SLA breaches for this date.</div>}
      </div>
    </section>
    ...
    <style>{`
      ...
      .tenant-dispatch-kpis{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:10px}.tenant-dispatch-kpi{display:flex;min-height:109px;flex-direction:column;gap:10px;padding:13px;border:1px solid var(--border);border-radius:15px;background:var(--surface)}.tenant-dispatch-kpi[data-tone=warning]{border-color:var(--warning-border)}.tenant-dispatch-kpi[data-tone=danger]{border-color:var(--danger-border)}.tenant-dispatch-kpi-label{display:flex;align-items:center;justify-content:space-between;gap:8px;color:var(--text-tertiary);font:500 10px/1.2 "IBM Plex Mono",var(--font-family-mono);letter-spacing:.08em;text-transform:uppercase}.tenant-dispatch-kpi-label i{display:grid;place-items:center;width:24px;height:24px;border-radius:8px;background:var(--surface-sunken);color:var(--text-tertiary);font-style:normal}.tenant-dispatch-kpi[data-tone=warning] .tenant-dispatch-kpi-label i{background:var(--warning-bg);color:var(--warning-text)}.tenant-dispatch-kpi[data-tone=danger] .tenant-dispatch-kpi-label i{background:var(--danger-bg);color:var(--danger-text)}.tenant-dispatch-kpi strong{color:var(--text-primary);font:650 28px/1 "IBM Plex Mono",var(--font-family-mono)}.tenant-dispatch-kpi small{color:var(--text-tertiary);font-size:11px}
      ...
      .tenant-dispatch-assigned-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px}.tenant-dispatch-assigned-card{display:flex;min-width:0;flex-direction:column;gap:7px;padding:13px;border:1px solid var(--border);border-radius:13px;background:var(--surface);color:var(--text-secondary);text-align:left;cursor:pointer}.tenant-dispatch-assigned-card:hover,.tenant-dispatch-assigned-card[aria-current=true]{border-color:var(--brand)}.tenant-dispatch-assigned-card[data-breach=true]{border-color:var(--danger-border);background:var(--danger-bg)}.tenant-dispatch-assigned-card>b{color:var(--text-primary);font-size:13px}.tenant-dispatch-assigned-card>span:not(.tenant-dispatch-job-top):not(.tenant-dispatch-assigned-meta){font-size:11.5px}.tenant-dispatch-assigned-meta{display:flex;align-items:center;justify-content:space-between;gap:8px;margin-top:2px;padding-top:8px;border-top:1px solid var(--border)}.tenant-dispatch-assigned-meta>span{display:flex;align-items:center;gap:5px;font-size:11.5px}.tenant-dispatch-assigned-meta>b{padding:4px 7px;border-radius:999px;background:var(--surface-sunken);color:var(--text-secondary);font-size:10px}.tenant-dispatch-assigned h2 i{background:var(--brand)}.tenant-dispatch-breached h2{color:var(--danger-text)}
      .tenant-dispatch-technician-list{display:flex;flex-direction:column;gap:10px}.tenant-dispatch-technician{padding:13px;border:1px solid var(--border);border-radius:13px;background:var(--surface)}.tenant-dispatch-technician>header{display:flex;align-items:center;gap:10px}.tenant-dispatch-avatar{display:grid;width:34px;height:34px;flex:none;place-items:center;overflow:hidden;border-radius:50%;background:var(--accent-muted);color:var(--brand);font-size:11px;font-weight:800}.tenant-dispatch-avatar img{width:100%;height:100%;object-fit:cover}.tenant-dispatch-technician-name{display:flex;min-width:0;flex:1;flex-direction:column;gap:2px}.tenant-dispatch-technician-name strong{color:var(--text-primary);font-size:13px}.tenant-dispatch-technician-name small{overflow:hidden;color:var(--text-tertiary);font-size:10.5px;text-overflow:ellipsis;white-space:nowrap}.tenant-dispatch-capacity{padding:4px 8px;border-radius:7px;background:var(--accent-muted);color:var(--brand);font:600 10px/1 "IBM Plex Mono",var(--font-family-mono)}.tenant-dispatch-slots{display:flex;gap:8px;flex-wrap:wrap;margin-top:9px}.tenant-dispatch-slots button,.tenant-dispatch-open-slot{display:flex;min-height:48px;flex-direction:column;justify-content:center;gap:2px;padding:7px 11px;border:1px solid var(--border);border-radius:10px;background:var(--surface);color:var(--text-primary);text-align:left}.tenant-dispatch-slots button{cursor:pointer}.tenant-dispatch-slots button:hover,.tenant-dispatch-slots button[aria-current=true]{border-color:var(--brand);background:var(--accent-muted)}.tenant-dispatch-slots button strong{font-size:11.5px}.tenant-dispatch-slots button span{font-size:10.5px}.tenant-dispatch-slots button small{color:var(--text-tertiary);font-size:9.5px}.tenant-dispatch-slots button small[data-overdue=true]{color:var(--danger-text);font-weight:700}.tenant-dispatch-open-slot{border-style:dashed;color:var(--text-tertiary);font-size:11px}.tenant-dispatch-empty{padding:30px;text-align:center;color:var(--text-tertiary);font-size:12px}
      @media(max-width:1000px){.tenant-dispatch-kpis{grid-template-columns:repeat(2,minmax(0,1fr))}}@media(max-width:820px){.tenant-dispatch-kpis{grid-template-columns:1fr}.tenant-dispatch-unassigned-grid,.tenant-dispatch-assigned-grid{grid-template-columns:1fr}.tenant-dispatch-section>header{align-items:flex-start;flex-direction:column}.tenant-dispatch-today>header p{max-width:460px}}
    `}</style>
  </div>;
}
