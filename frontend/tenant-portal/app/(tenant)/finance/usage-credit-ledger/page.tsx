...
export default function UsageCreditLedgerPage() {
  ...
  return (
    <TenantLayout activeNav="finance-credit-ledger">
      <PageHeader
        title="Usage Credits"
        description="Track your Home Services usage credit balance, completed job deductions, and credit activity."
      />

      {(balance.error || ledger.error) && (
        <div style={{ background: "var(--danger-bg)", border: "1px solid var(--danger-border)", borderRadius: 12, padding: 20, margin: "16px 0", color: "var(--danger-text)" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 14, margin: "16px 0 24px" }}>
        {[
          ...
          <Card key={card.label}>
            ...
            <p style={{ fontSize: "1.5rem", fontWeight: 700, color: card.label === "Low Credit Status" && balance.data?.low_credit ? "var(--danger-text)" : "var(--brand)", margin: 0 }}>{card.value}</p>
          </Card>
        ))}
      </div>
      ...
      {ledger.loading ? (
        <Skeleton height="12.5rem" radius="12px" />
      ) : entries.length === 0 ? (
        <Card style={{ padding: 48, textAlign: "center", color: "var(--text-secondary)" }}>
            ...
            <tbody>
              {entries.map((entry) => (
                <tr key={entry.ledger_id} style={{ borderTop: "1px solid var(--border)" }}>
                  ...
                  <td style={{ padding: "10px 14px", fontSize: "0.875rem", fontWeight: 600, color: entry.credit_delta < 0 ? "var(--danger-text)" : "var(--success)" }}>
