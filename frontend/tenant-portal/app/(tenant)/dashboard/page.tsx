...
function OperationalDashboard() {
  ...
  return <PageShell>
    <PageHeader title="Operations dashboard" description="Your Home Services command centre." />
    <style jsx global>{`
      .dashboard-main-grid{display:grid;grid-template-columns:minmax(0,1.7fr) minmax(290px,1fr);gap:14px;align-items:start}.dashboard-split-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:14px}.dashboard-pipeline-bar{display:flex;align-items:stretch;gap:2px;height:10px;overflow:hidden;border-radius:11px;background:var(--surface-sunken)}.dashboard-pipeline-tiles{display:grid;grid-template-columns:repeat(auto-fill,minmax(min(150px,100%),1fr));gap:8px}.dashboard-pipeline-tile{display:flex;align-items:center;gap:9px;padding:9px 10px;border:1px solid var(--border);border-radius:11px;background:var(--surface-sunken)}.dashboard-pipeline-tile span:nth-child(2){flex:1;min-width:0;color:var(--text-secondary);font-size:12px}.dashboard-pipeline-tile strong{font-family:"IBM Plex Mono",monospace;font-size:15px;color:var(--text-primary)}.dashboard-pipeline-dot{width:9px;height:9px;flex:none;border-radius:3px}.dashboard-row-button{display:flex;width:100%;align-items:center;gap:11px;padding:11px 0;background:none;border:0;border-bottom:1px solid var(--border);cursor:pointer;text-align:left;font:inherit}.dashboard-row-button:last-child{border-bottom:0}.dashboard-attention-icon{width:34px;height:34px;border-radius:10px;display:grid;place-items:center;flex-shrink:0}.dashboard-attention-danger{background:var(--danger-bg);color:var(--danger-text)}.dashboard-attention-warning{background:var(--warning-bg);color:var(--warning-text)}.dashboard-attention-info{background:var(--info-bg);color:var(--info-text)}.dashboard-row-title{display:block;color:var(--text-primary);font-size:13px;font-weight:650}.dashboard-row-detail{display:block;color:var(--text-tertiary);font-size:11px;margin-top:3px}.dashboard-metric-icon{width:38px;height:38px;border-radius:11px;display:grid;place-items:center;flex-shrink:0}.dashboard-finance-tile{padding:12px;border:1px solid var(--border);border-radius:11px;background:var(--surface-sunken)}.dashboard-finance-tile strong{display:block;color:var(--text-primary);font-family:"IBM Plex Mono",monospace;font-size:17px;margin-top:8px}.dashboard-finance-tile span{display:block;color:var(--text-tertiary);font-size:10px;margin-top:3px}.dashboard-table-wrap{overflow-x:auto}.dashboard-table{width:100%;border-collapse:collapse;min-width:680px}.dashboard-table th{padding:9px 12px;text-align:left;font-size:10px;text-transform:uppercase;letter-spacing:.06em;color:var(--text-tertiary);background:var(--surface-sunken);border-bottom:1px solid var(--border)}.dashboard-table td{padding:10px 12px;font-size:12px;color:var(--text-secondary);border-bottom:1px solid var(--border)}.dashboard-table tbody tr{cursor:pointer}.dashboard-table tbody tr:hover{background:var(--surface-sunken)}.dashboard-table tbody tr:last-child td{border-bottom:0}@media(max-width:1050px){.dashboard-main-grid{grid-template-columns:1fr}}@media(max-width:700px){.dashboard-split-grid{grid-template-columns:1fr}}
    `}</style>
    ...
    <KpiGrid minCardWidth={180}>
      ...
      <SummaryCard icon={<CheckCircle2 />} label="Provider health" value={data.provider_health.score == null ? "—" : `${Math.round(data.provider_health.score)}/100`} sub={data.provider_health.band ? `${humanize(data.provider_health.band)} · ${Math.round(data.provider_health.history_window_days / 30)}-month rolling` : `Building ${Math.round(data.provider_health.history_window_days / 30)}-month history`} tone={(data.provider_health.score ?? 100) < 60 ? "warning" : "success"} />
    ...
    <div className="dashboard-main-grid"><div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      <Card title="Job pipeline" actions={<Button variant="link" onClick={() => router.push("/home-services/bookings-jobs")}>Manage pipeline</Button>}>
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          <div className="dashboard-pipeline-bar" aria-label={`${count(pipelineTotal)} jobs across the pipeline`}>
            {data.job_pipeline.map((stage, index) => <span key={stage.key} title={`${stage.label}: ${count(stage.count)}`} style={{ flex: Math.max(stage.count, .12), background: pipelineColors[index % pipelineColors.length] }} />)}
          </div>
          <div className="dashboard-pipeline-tiles">{data.job_pipeline.map((stage, index) => <div className="dashboard-pipeline-tile" key={stage.key}>
            <span className="dashboard-pipeline-dot" style={{ background: pipelineColors[index % pipelineColors.length] }} />
            <span>{stage.label}</span><strong>{count(stage.count)}</strong>
          </div>)}</div>
        </div>
      </Card>
