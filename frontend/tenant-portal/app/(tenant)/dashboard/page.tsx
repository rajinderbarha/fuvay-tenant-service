...
export default function DashboardPage() {
  ...
  if (access !== "allowed") return <PageShell><PageHeader title="Loading your workspace" description="Checking your Home Services setup status." /><Skeleton height={180} /></PageShell>;
...
function OperationalDashboard() {
  ...
  if (dashboard.loading && !data) return <PageShell><PageHeader title="Operations dashboard" description="Loading live jobs, staff capacity, customers and finance." /><KpiGrid minCardWidth={210}>{[1, 2, 3, 4].map((key) => <Skeleton key={key} height={132} />)}</KpiGrid><Skeleton height={320} /></PageShell>;
  if (dashboard.error || !data) return <PageShell><PageHeader title="Operations dashboard" description="Your Home Services command centre." /><Alert tone="danger" title="Dashboard could not be loaded">{dashboard.error ?? "The server returned no dashboard data."}{dashboard.requestId ? ` Request ID: ${dashboard.requestId}` : ""}</Alert><Button variant="secondary" leftIcon={<RefreshCw size={16} />} onClick={dashboard.refetch}>Try again</Button></PageShell>;
  ...
  return <PageShell>
    ...
    `}</style>
    <PageHeader title={`${data.workspace.business_name} operations`} description="Live Home Services jobs, technician capacity, customer quality and finance in one workspace." actions={<div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}><Button variant="secondary" leftIcon={<Wrench size={16} />} onClick={() => router.push("/home-services/dispatch")}>Open dispatch</Button><Button variant="secondary" leftIcon={<RefreshCw size={16} />} loading={dashboard.loading} onClick={dashboard.refetch}>Refresh</Button></div>} />
    ...
    {!data.bookability.is_bookable && data.bookability.blockers.length > 0 && <Alert tone="danger" title="Customers cannot book this workspace">{data.bookability.blockers.map((blocker) => blocker.message ?? humanize(blocker.code ?? "Setup requirement incomplete")).join(" · ")}</Alert>}
    ...
    <div className="dashboard-main-grid"><div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      <Card title="Staff capacity" actions={<Button variant="link" onClick={() => router.push("/home-services/team")}>Manage team</Button>}><div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 8 }}>{([['Ready', data.staff_capacity.ready], ['Assigned', data.staff_capacity.assigned_now], ['Available', data.staff_capacity.available_now]] as const).map(([label, value]) => <div key={label} style={{ padding: 10, borderRadius: 10, background: "var(--surface-sunken)" }}><strong style={{ display: "block", color: "var(--text-primary)", fontSize: 18 }}>{count(value)}</strong><span style={{ color: "var(--text-tertiary)", fontSize: 10 }}>{label}</span></div>)}</div><div style={{ marginTop: 12, display: "flex", flexDirection: "column", gap: 7 }}>{data.service_bookability.slice(0, 5).map((service) => <div key={service.offering_id} style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 11 }}><span style={{ flex: 1, color: "var(--text-secondary)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{service.name}</span><Badge size="sm" variant={service.bookable ? "success" : "warning"}>{service.bookable ? "Bookable" : service.blocking_reason ?? "Blocked"}</Badge></div>)}{data.service_bookability.length === 0 && <span style={{ color: "var(--text-tertiary)", fontSize: 12 }}>No published offerings found.</span>}</div></Card>
    </div></div>
