...
import {
  ...
  ExternalLink, Filter, LayoutList, MapPin, RefreshCw, Search, ShieldAlert, Table2,
    ...
    <Card padding="none">
      <div style={{ padding: 16, borderBottom: "1px solid var(--border)", display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap" }}>
        ...
        <div className="bj-view-switch" aria-label="Bookings view">
          ...
          <button type="button" aria-label="Table view" aria-pressed={viewMode === "table"} onClick={() => setViewMode("table")}><Table2 size={16} /></button>
        </div>
      </div>
...
function JobCard({ row, selected, onOpen }: { row: BJItem; selected: boolean; onOpen: () => void }) {
  return <button type="button" className="bj-job-card" aria-current={selected} onClick={onOpen}>
    <span className="bj-card-top">
      ...
      <span className="bj-card-title"><span><strong>{row.job_number}</strong><StatusBadge status={row.stage} size="sm" /></span><span className="bj-card-id">{row.booking_number}</span><span className="bj-card-service">{row.service_name ?? "Service unavailable"}</span></span>
