...
function BookingsJobsWorkspace() {
  ...
  return <><PageShell>
    ...
    {list.loading && !summary ? <KpiSkeleton /> : summary && <KpiGrid minCardWidth={155}>
      ...
      <SummaryCard label="SLA attention" value={summary.at_risk} sub="At risk or breached" icon={<ShieldAlert />} tone={summary.at_risk ? "danger" : undefined} onClick={() => updateFilter("sla", "ATTENTION")} active={sla === "ATTENTION"} />
...
function AdvancedFilters({ data, values, onChange, onDateChange }: { data: BJListResponse | null; values: Record<string, string>; onChange: (key: string, value: string) => void; onDateChange: (key: string, value: string) => void }) {
  return <div style={{ padding: 16, background: "var(--surface-sunken)", borderBottom: "1px solid var(--border)" }}><div className="bj-filter-grid">
    ...
    <FilterSelect label="SLA" value={values.sla} onChange={v => onChange("sla", v)}><option value="">Any SLA</option><option value="ATTENTION">Needs attention</option><option value="ON_TRACK">On track</option><option value="AT_RISK">At risk</option><option value="BREACHED">Breached</option></FilterSelect>
