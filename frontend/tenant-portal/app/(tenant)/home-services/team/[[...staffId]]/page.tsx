...
function JobsTab({ staffId }: { staffId: string }) {
  ...
  return <Card padding="none"><div className="team-toolbar"><label className="team-search"><Search size={15} /><input value={search} onChange={event => setSearch(event.target.value)} placeholder="Search job or service" /></label><Filter label="Status" value={status} onChange={setStatus} options={result.data?.available_statuses ?? []} /></div>{result.loading ? <div className="team-loading"><Skeleton height={52} /><Skeleton height={52} /></div> : result.error ? <Alert tone="danger">{result.error}</Alert> : result.data?.items.length ? <div className="team-table-wrap"><table className="team-table"><thead><tr><th>Job</th><th>Service</th><th>Schedule</th><th>Status</th><th></th></tr></thead><tbody>{result.data.items.map((job: any) => <tr key={job.id} onClick={() => location.href = `/home-services/bookings-jobs?job_id=${job.id}`}><td><strong>{job.job_number}</strong><span className="team-cell-note">{job.city || "Location pending"}</span></td><td>{job.service_name}</td><td>{dateText(job.scheduled_date)}<span className="team-cell-note">{job.scheduled_time_window || "Time pending"}</span></td><td><StatusBadge status={job.status} size="sm" /></td><td><ChevronRight size={16} /></td></tr>)}</tbody></table></div> : <InlineEmpty text="No assigned jobs match this view." />}{result.data && <div className="team-pagination"><span>{result.data.pagination.total} jobs</span><Button variant="secondary" size="sm" disabled={!cursor} onClick={() => setCursor(result.data?.pagination.previous_cursor ?? 0)}><ChevronLeft size={15} /></Button><Button variant="secondary" size="sm" disabled={result.data.pagination.next_cursor == null} onClick={() => setCursor(result.data?.pagination.next_cursor ?? cursor)}><ChevronRight size={15} /></Button></div>}</Card>;
}
