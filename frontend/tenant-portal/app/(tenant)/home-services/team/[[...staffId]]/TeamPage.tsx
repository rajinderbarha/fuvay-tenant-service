...
function TeamDirectory() {
  ...
  return <><PageShell>
    ...
    <section className="team-kpis" aria-label="Team summary">{directory.loading || !summary
      ? Array.from({ length: 5 }).map((_, index) => <Skeleton key={index} height={104} />) : <>
    ...
    <div className="team-insight-grid"><Card><div className="team-insight"><span className="team-insight-icon"><ShieldCheck size={18} /></span><div><strong>Setup and dispatch use this same roster</strong><p>Services and schedules resolve from these same records.</p></div><Link href="/tenant/home-services/setup/staff">Review →</Link></div></Card><Card><div className="team-insight"><span className="team-insight-icon"><Wallet size={18} /></span><div><strong>Workforce funding impact</strong><p>{funding.loading ? "Checking finance policy…" : fundingQuote ? `${fundingQuote.qualifying_technician_count ?? 0} qualifying technicians · ${seatsOwned ?? 0} seat${seatsOwned === 1 ? "" : "s"} purchased` : "Finance policy is shown in the canonical Home Services finance workspace."}</p></div><Link href="/home-services/finance">Open →</Link></div></Card></div>
    <Card padding="none"><div className="team-toolbar"><label className="team-search"><Search size={15} /><input aria-label="Search team" value={searchInput} onChange={e => setSearchInput(e.target.value)} placeholder="Search name, email, phone or designation" /></label><Filter label="Role" value={role} onChange={setRole} options={directory.data?.available_filters.role ?? ["technician", "staff", "manager"]} /><Filter label="Status" value={status} onChange={setStatus} options={directory.data?.available_filters.status ?? ["active", "inactive"]} /><Filter label="Availability" value={availability} onChange={setAvailability} options={directory.data?.available_filters.availability ?? ["available", "busy", "offline", "unavailable"]} /><Filter label="Readiness" value={readiness} onChange={setReadiness} options={directory.data?.available_filters.readiness ?? ["ready", "needs_setup", "invitation_pending", "access_disabled"]} />{hasFilters && <Button variant="ghost" size="sm" leftIcon={<FilterX size={14} />} onClick={clearFilters}>Clear</Button>}</div>
      <div className="team-result-line"><span>{directory.loading ? "Loading roster…" : `${directory.data?.pagination.total ?? 0} team member${directory.data?.pagination.total === 1 ? "" : "s"}`}</span></div>
...
function TeamRow({ row, onOpen }: { row: TeamDirectoryStaffRow; onOpen: () => void }) {
  const capacity = row.capacity_limit ? Math.min(100, row.capacity_used / row.capacity_limit * 100) : 0;
  return <button type="button" className="team-roster-row" onClick={onOpen}><span className="team-person"><Avatar src={row.photo} name={row.name} size={42} /><span><strong>{row.name}</strong><span>{row.verification_status === "no_login_configured" ? "Profile only · no app login" : titleCase(row.verification_status)}</span></span></span><span className="team-roster-field"><small>Role</small><strong>{titleCase(row.role)}</strong><span>{titleCase(row.employment_status)}</span></span><span className="team-roster-field"><small>Availability</small><StatusBadge status={row.on_leave ? "on_leave" : row.availability_status} size="sm" /><span>{row.on_leave ? "Approved time off" : row.conflict_count ? `${row.conflict_count} conflict(s)` : "Presence synced"}</span></span><span className="team-roster-field"><small>Today</small><strong>{row.jobs_today} job{row.jobs_today === 1 ? "" : "s"}</strong><span className="team-capacity-bar"><i style={{ width: `${capacity}%` }} /></span><span>{row.capacity_used}/{row.capacity_limit} capacity</span></span><span className="team-roster-field"><small>Readiness</small><StatusBadge status={row.readiness} size="sm" /><span>{row.readiness_missing.length ? row.readiness_missing.map(titleCase).join(", ") : "Ready for assignment"}</span></span><ChevronRight size={17} /></button>;
}
...
function StaffDetail({ staffId, tab, onTabChange }: { staffId: string; tab: TabKey; onTabChange: (tab: TabKey) => void }) {
  ...
  return <div className="team-detail"><Card>{overview.loading || !overview.data ? <Skeleton height={90} /> : overview.error ? <Alert tone="danger">{overview.error}</Alert> : <div className="team-profile-head"><Avatar src={overview.data.identity.photo} name={overview.data.identity.name} size={62} /><div className="team-profile-copy"><div className="team-profile-title"><h1>{overview.data.identity.name}</h1><StatusBadge status={overview.data.employment_status} size="sm" /></div><p>{overview.data.identity.designation || titleCase(overview.data.identity.role)}</p><div><StatusBadge status={overview.data.verification_status} size="sm" /><StatusBadge status={overview.data.availability_status} size="sm" /><StatusBadge status={overview.data.readiness.status} size="sm" /></div></div><Button variant="secondary" leftIcon={<Pencil size={14} />} onClick={() => setEditOpen(true)} disabled={!member.data}>Edit profile</Button></div>}</Card><nav className="team-tabs" aria-label="Team member sections">{TABS.map(([key, label]) => <button key={key} className={tab === key ? "active" : ""} onClick={() => onTabChange(key)}>{label}</button>)}</nav>
    ...
    <Button variant="secondary" disabled={!member.data} onClick={() => { setEditSection(4); setEditOpen(true); }}>Manage login &amp; access</Button>
    {editOpen && member.data && <AddTeamMemberWizard initialSection={editSection} existing={member.data as ProviderTeamMember} onClose={() => { setEditOpen(false); setEditSection(0); member.refetch(); overview.refetch(); }} onSaved={() => { setEditOpen(false); setEditSection(0); member.refetch(); overview.refetch(); }} />}</div>;
}
...
function TeamStyles() { return <style jsx global>{`
  ...
  .team-roster-list{display:flex;flex-direction:column}.team-roster-row{display:grid;grid-template-columns:minmax(210px,1.3fr) repeat(4,minmax(130px,1fr)) 20px;align-items:center;gap:14px;width:100%;padding:14px 16px;border:0;border-top:1px solid var(--border);background:var(--surface);color:var(--text-secondary);text-align:left;transition:background .15s}.team-roster-row:hover{background:var(--surface-sunken)}.team-roster-row .team-person>span:last-child{display:flex;min-width:0;flex-direction:column;gap:3px}.team-roster-field{display:flex;min-width:0;flex-direction:column;align-items:flex-start;gap:4px}.team-roster-field small{color:var(--text-tertiary);font:500 10px/1 "IBM Plex Mono",var(--font-family-mono);letter-spacing:.055em;text-transform:uppercase}.team-roster-field strong{color:var(--text-primary);font-size:12px}.team-roster-field>span:last-child{overflow:hidden;max-width:100%;color:var(--text-tertiary);font-size:10.5px;text-overflow:ellipsis;white-space:nowrap}.team-capacity-bar{display:block;width:90px;height:5px;border-radius:99px;background:var(--surface-sunken);overflow:hidden}.team-capacity-bar i{display:block;height:100%;border-radius:inherit;background:var(--brand)}
  .team-detail,.team-stack{display:flex;flex-direction:column;gap:16px}.team-detail>.ds-surface-card:first-child{padding:16px!important}.team-profile-head{display:flex;align-items:center;gap:14px}.team-profile-copy{flex:1;min-width:0}.team-profile-title{display:flex;gap:8px;align-items:center}.team-profile-title h1{margin:0;color:var(--text-primary);font-size:21px;letter-spacing:-.025em}.team-profile-copy>p{margin:4px 0 7px;color:var(--text-secondary);font-size:12.5px}.team-profile-copy>div:last-child{display:flex;gap:6px;flex-wrap:wrap}.team-tabs{display:flex;gap:6px;overflow-x:auto;border-bottom:1px solid var(--border)}.team-tabs button{padding:11px 4px;border:0;border-bottom:2px solid transparent;background:none;color:var(--text-tertiary);font:500 13px/1 inherit;white-space:nowrap;cursor:pointer}.team-tabs button:hover{color:var(--text-primary)}.team-tabs button.active{color:var(--text-primary);border-bottom-color:var(--brand);font-weight:600}
  .team-info{display:grid;grid-template-columns:30px minmax(72px,.42fr) 1fr;align-items:center;gap:12px;padding:8px 0;border-bottom:1px solid var(--border)}.team-info:last-child{border-bottom:0}.team-info>span{display:grid;place-items:center;width:30px;height:30px;border-radius:9px;background:var(--surface-sunken);color:var(--text-tertiary)}.team-info small{color:var(--text-tertiary);font:500 10px/1 "IBM Plex Mono",var(--font-family-mono);letter-spacing:.06em;text-transform:uppercase}.team-info strong{color:var(--text-primary);font-size:13px;font-weight:600}.team-list-row{display:flex;align-items:center;justify-content:space-between;gap:12px;padding:9px 0;border-bottom:1px solid var(--border);color:var(--text-secondary);text-decoration:none}.team-list-row:last-child{border-bottom:0}.team-list-row>span{display:flex;flex-direction:column;gap:3px}.team-list-row strong{color:var(--text-primary);font-size:12px}.team-list-row small{color:var(--text-tertiary);font-size:10.5px}.team-list-row:hover strong{color:var(--brand)}.team-inline-empty{padding:22px 0;text-align:center;color:var(--text-tertiary);font-size:12px}.team-chips{display:flex;gap:7px;flex-wrap:wrap}.team-chips>span{padding:8px 12px;border-radius:999px;border:1px solid var(--border);background:var(--surface-sunken);color:var(--text-secondary);font-size:12px}.team-capacity{text-align:center}.team-capacity>strong{display:block;color:var(--text-primary);font:700 34px/1 "IBM Plex Mono",var(--font-family-mono)}.team-capacity>span{color:var(--text-tertiary);font-size:12px}.team-capacity>div,.team-metric>div:last-child{height:6px;margin-top:12px;border-radius:999px;background:var(--surface-sunken);overflow:hidden}.team-capacity i,.team-metric i{display:block;height:100%;border-radius:inherit;background:var(--brand)}.team-check{display:flex;align-items:center;gap:9px;padding:6px 0;color:var(--text-secondary);font-size:13px}.team-check svg{color:var(--success)}
  ...
  @media(max-width:1180px){.team-kpis{grid-template-columns:repeat(3,1fr)}.team-roster-row{grid-template-columns:minmax(210px,1.3fr) repeat(2,minmax(140px,1fr)) 20px}.team-roster-field:nth-of-type(4),.team-roster-field:nth-of-type(5){display:none}}@media(max-width:850px){.team-insight-grid,.team-detail-grid{grid-template-columns:1fr}.team-capability{grid-template-columns:1fr}.team-profile-head{align-items:flex-start;flex-wrap:wrap}.team-profile-head>button{width:100%}.team-roster-row{grid-template-columns:minmax(180px,1fr) minmax(130px,.7fr) 20px}.team-roster-field{display:none!important}.team-roster-field:nth-of-type(3){display:flex!important}}@media(max-width:620px){.team-kpis,.team-kpis-four{grid-template-columns:repeat(2,1fr)}.team-toolbar{align-items:stretch}.team-toolbar>*{flex:1 1 145px}.team-search{min-width:100%}.team-result-line>span:last-child{display:none}.team-insight{align-items:flex-start;flex-wrap:wrap}.team-insight a{margin-left:50px}.team-pagination{flex-wrap:wrap}.team-roster-row{grid-template-columns:1fr 20px}.team-roster-field{display:none!important}}
  .team-roster-field>small{display:none}
  @media(max-width:1180px){.team-kpis{grid-template-columns:repeat(3,1fr)}}
  @media(max-width:620px){.team-kpis,.team-kpis-four{grid-template-columns:repeat(2,1fr)}}
`}</style>; }
