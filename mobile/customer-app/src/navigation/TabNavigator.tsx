...
function TabIcon({ icon, label, focused }: { icon: string; label: string; focused: boolean }) {
  return (
    <View style={{ alignItems: "center", gap: 2, paddingTop: 6 }}>
      ...
      <Text style={{ fontSize: 9, fontWeight: "600", color: focused ? theme.colors.tabActive : theme.colors.tabInactive }}>{label}</Text>
    </View>
  );
}
  ...
  return (
    <Tab.Navigator
      screenOptions={{
        ...
        tabBarStyle: { backgroundColor: "#fff", borderTopWidth: 1, borderTopColor: theme.colors.border, height: 64, paddingBottom: 8 },
