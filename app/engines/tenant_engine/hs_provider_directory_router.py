...
@router.get("/export", response_model=ApiResponse[dict], summary="Export Home Services providers")
async def hs_providers_export(r: Request, q: str | None = Query(None), status: str | None = Query(None),
                               u: UserContext = Depends(require_permission(P.HOME_SERVICES_PROVIDERS_EXPORT)),
                               db: AsyncSession = Depends(get_db)):
    s = HomeServicesProviderDirectoryService(db=db, request_id=_rid(r),
                                              actor_id=uuid.UUID(u.user_id) if u.user_id else None, actor_role=u.role)
    data = await s.list_providers(q=q, status=status, page=1, page_size=5000)
...
async def hs_provider_team(provider_id: uuid.UUID, r: Request,
                            s: HomeServicesProviderDirectoryService = Depends(_svc)):
    return ok(await s.get_provider_team(provider_id), _rid(r), ENGINE_ID)
