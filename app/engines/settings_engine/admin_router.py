...
@router.get("/plans", summary="List packages with settings-relevant fields")
async def list_plans(r: Request,
                      u: UserContext = Depends(require_permission(P.SETTINGS_PLAN_READ)),
                      db: AsyncSession = Depends(get_db)):
    from app.engines.package_commerce.models import ServicePackage
    rows = (await db.execute(select(ServicePackage).where(
        ServicePackage.deleted_at.is_(None)).order_by(ServicePackage.display_order))).scalars().all()
    return ok({"packages": [_package_to_dict(p) for p in rows]}, _rid(r), ENGINE_ID)


@router.get("/plans/{package_id}", summary="Get package settings")
async def get_plan(r: Request, package_id: uuid.UUID,
                    u: UserContext = Depends(require_permission(P.SETTINGS_PLAN_READ)),
                    db: AsyncSession = Depends(get_db)):
    from app.engines.package_commerce.models import ServicePackage, PackageLimit
    pkg = (await db.execute(select(ServicePackage).where(ServicePackage.id == package_id))).scalar_one_or_none()
    if not pkg: raise NotFoundException("ServicePackage", str(package_id))
    limits = (await db.execute(select(PackageLimit).where(PackageLimit.package_id == package_id))).scalars().all()
    return ok({**_package_to_dict(pkg), "limits": [
        {"limit_key": l.limit_key, "limit_label": l.limit_label,
         "limit_value": float(l.limit_value) if l.limit_value is not None else None} for l in limits
    ]}, _rid(r), ENGINE_ID)


class UpdatePlanBody(BaseModel):
    included_credit_amount: Optional[float] = None
...
@router.put("/plans/{package_id}", summary="Update package settings")
async def update_plan(r: Request, package_id: uuid.UUID, body: UpdatePlanBody,
                       u: UserContext = Depends(require_permission(P.SETTINGS_PLAN_UPDATE)),
                       db: AsyncSession = Depends(get_db)):
    from app.engines.package_commerce.models import ServicePackage, PackageAuditLog
    pkg = (await db.execute(select(ServicePackage).where(ServicePackage.id == package_id))).scalar_one_or_none()
    if not pkg: raise NotFoundException("ServicePackage", str(package_id))
    before = _package_to_dict(pkg)
    for field in ("included_credit_amount", "storage_quota_gb", "commission_rate",
                  "security_deposit_amount", "validity_days", "is_active"):
        val = getattr(body, field)
        if val is not None:
            setattr(pkg, field, Decimal(str(val)) if field in
                    ("included_credit_amount", "storage_quota_gb", "commission_rate", "security_deposit_amount")
                    else val)
    db.add(PackageAuditLog(package_id=package_id, actor_user_id=uuid.UUID(u.user_id) if u.user_id else None,
                            actor_role=u.role, action="settings.plan_updated",
                            old_value=before, new_value=_package_to_dict(pkg), reason=body.reason))
    return ok(_package_to_dict(pkg), _rid(r), ENGINE_ID)


def _package_to_dict(p) -> dict:
    return {
        "id": str(p.id), "name": p.name, "package_type": p.package_type, "plan_level": p.plan_level,
        "billing_cycle": p.billing_cycle, "currency": p.currency,
        "included_credit_amount": float(p.included_credit_amount) if p.included_credit_amount is not None else None,
        ...
        "security_deposit_amount": float(p.security_deposit_amount) if p.security_deposit_amount is not None else None,
        "validity_days": p.validity_days, "is_active": p.is_active, "vertical_type": p.vertical_type,
    }
