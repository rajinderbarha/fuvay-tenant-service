...
# A technician picks every part from the provider's inventory; name and
# customer price come from that catalogue, never typed in the app.
ERR_PARTS_INVENTORY_ITEM_REQUIRED    = "PARTS_INVENTORY_ITEM_REQUIRED"
ERR_PARTS_INVENTORY_ITEM_UNAVAILABLE = "PARTS_INVENTORY_ITEM_UNAVAILABLE"
ERR_PARTS_INSUFFICIENT_STOCK         = "PARTS_INSUFFICIENT_STOCK"
# Work cannot be marked done while a part still waits on a decision; the
# technician cancels the request if the customer never answers.
ERR_PARTS_PENDING_BLOCK_WORK_DONE    = "PARTS_PENDING_BLOCK_WORK_DONE"
PARTS_PENDING_STATUSES = frozenset({PARTS_STATUS_REQUESTED, PARTS_STATUS_CUSTOMER_APPROVAL_PENDING})
PARTS_APPROVED_STATUSES = frozenset({PARTS_STATUS_BUSINESS_APPROVED, PARTS_STATUS_CUSTOMER_APPROVED})
EV_PARTS_REQUEST_CANCELLED = "parts_request_cancelled"
