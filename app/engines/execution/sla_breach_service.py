...
async def sweep(db: AsyncSession, *, limit: int = 200) -> dict:
    ...
    penalty = Decimal(str(policy.sla_penalty_amount or 0))
    ...
    due = (await db.execute(text(
        "SELECT id, tenant_id, booking_id, customer_id, status "
        "FROM service_jobs "
        "WHERE sla_due_at IS NOT NULL AND sla_breached_at IS NULL "
        "  AND sla_due_at <= now() AND status = ANY(:statuses) "
        "ORDER BY sla_due_at LIMIT :lim FOR UPDATE SKIP LOCKED"
    ), {"statuses": list(BREACHABLE_STATUSES), "lim": limit})).fetchall()

    breached = penalised = compensated = 0
    for job in due:
        # 1. Release the customer first. Everything after this is about money,
        #    and none of it should delay letting them rebook.
        await db.execute(text(
