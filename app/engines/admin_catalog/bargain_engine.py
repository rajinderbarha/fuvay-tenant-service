...
def round_to_nearest_5(value) -> Decimal:
    v = _d(value)
    return (v / Decimal("5")).quantize(Decimal("1"), rounding=ROUND_HALF_UP) * Decimal("5")


def round_to_nearest(value, increment: int) -> Decimal:
    v = _d(value)
    inc = Decimal(str(increment))
    return (v / inc).quantize(Decimal("1"), rounding=ROUND_HALF_UP) * inc


def compute_symmetric_customer_price_tiers(
    provider_min_price,
    provider_max_price,
    platform_fee_percent=0,
    platform_fee_fixed_amount=0,
    rounding_increment: int = 10,
) -> dict:
    """Admin/tenant Home Services Customer Price Options preview.

    Distinct from compute_bargain_floor/evaluate_customer_bargain (used by the
    certified provider-first-matching flow, which only adds the platform fee
    to the low end). This one adds the fee to BOTH ends of the provider's
    price range — the formula the Admin Catalog Console and Tenant Setup
    Wizard tickets both specify:

        Low  = provider_min * (1 + fee%/100) + fee_fixed
        High = provider_max * (1 + fee%/100) + fee_fixed
        Mid  = round_to_nearest(rounding_increment, (Low + High) / 2)

    e.g. provider range ₹550-₹700 @ 10% → Low ₹605, Mid ₹690, High ₹770
    (default rounding_increment=10 — used by the Admin Catalog Console and
    Tenant Setup Wizard, both already certified against nearest-10 output).
    The Customer Price Experience admin preview tool passes
    rounding_increment=5 instead, per its own ticket's exact example
    (₹385/₹425/₹462).
    Hard gate: never returns provider_min unmodified as Low.
    """
    if provider_min_price is None or provider_max_price is None:
        raise BargainValidationError(
            "PRICE_RANGE_REQUIRED", "Both provider_min_price and provider_max_price are required.")

    p_min = _d(provider_min_price)
    p_max = _d(provider_max_price)
    fee_pct = _d(platform_fee_percent or 0)
    fee_fixed = _d(platform_fee_fixed_amount or 0)

    if p_min < 0 or p_max < 0:
        raise BargainValidationError("INVALID_PRICE_RANGE", "Prices cannot be negative.")
    if p_min > p_max:
        raise BargainValidationError("INVALID_PRICE_RANGE", "provider_min_price cannot exceed provider_max_price.")
    if fee_pct < 0 or fee_fixed < 0:
        raise BargainValidationError("INVALID_PLATFORM_FEE", "Platform fee cannot be negative.")

    fee_multiplier = Decimal("1") + fee_pct / Decimal("100")
    low = _round2(p_min * fee_multiplier + fee_fixed)
    high = _round2(p_max * fee_multiplier + fee_fixed)
    mid = round_to_nearest((low + high) / Decimal("2"), rounding_increment)
    if mid < low:
        mid = low
    if mid > high:
        mid = high
    ...
    return {
        "provider_min_price": float(p_min),
        "provider_max_price": float(p_max),
        "platform_fee_percent": float(fee_pct),
        "platform_fee_fixed_amount": float(fee_fixed),
        "low_price": float(low),
        "mid_price": float(mid),
